<template>
  <div>
    <md-toolbar
      :class="{ 'md-accent': error, 'md-primary': !error, 'md-dense': true }"
      v-if="error"
    >
    ...
    >
      ...
      >
        <md-card-header>
          <img src="@/assets/logo-sem_fundo - Lucas.png" class="imagem" />
        </md-card-header>
        ...
        <md-card-content>
          <div class="md-layout md-gutter">
            <div class="md-layout-item md-small-size-100">
              <md-field
                :class="{ 'md-invalid': $v.credentialsUser.username.$error }"
              >
                ...
                <span
                  class="md-error"
                  v-if="!$v.credentialsUser.username.required"
                  >Campo obrigatório</span
                >
                <span
                  class="md-error"
                  v-else-if="!$v.credentialsUser.username.minLength"
                  >Campo deve ter no mínimo 3 caracteres</span
                >
              </md-field>
            </div>
            ...
            <div class="md-layout-item md-small-size-100">
              <md-field
                :class="{
                  'md-invalid': $v.credentialsUser.user_password.$error,
                }"
              >
                ...
                <span
                  class="md-error"
                  v-if="!$v.credentialsUser.user_password.required"
                  >Campo obrigatório</span
                >
                <span
                  class="md-error"
                  v-else-if="!$v.credentialsUser.user_password.minLength"
                  >Campo deve ter no mínimo 8 caracteres</span
                >
              </md-field>
            </div>
          </div>
        ...
        <router-link to="/create-account"
          ><md-button class="md-primary">Criar conta</md-button></router-link
        >
        <md-button
          type="submit"
          class="md-accent"
    ...
    logar: async function() {
      ...
      await UserService.login(this.credentialsUser)
        .then((response) => {
          ...
          if (
            this.isTrue(response.data, "Logado com sucesso") &&

<template>
    ...
    <div>
    <keep-alive>
        <component v-if="filterActive" v-bind:is="filterDashboard" @filterTableMain="toggleFilter($event)" />
    </keep-alive>
        <md-button class="md-fab md-mini md-plain" @click="toggleFilter()" style="display: flex">
...
export default {
    ...
    methods: {
        ...
        getDatasGrafics($event) {
            if($event?.component) {
                dashboarService.filter($event.component.filter)
                .then((response) => {
                    ...
                    if ("Dashboard_error" in response.data) 
                    {
                        ...
                        console.log(this.graficBar)
                    } 
                }).catch((error) => {
                    console.log(error)
                })
                
                 dashboarService.filterLine($event.component.filter)
                .then((response) => {
                    console.log(response.data)
                    if ("Dashboard_error" in response.data) 
                    {
